...
- 加固：`show_images` 工具防漏贴增强。实测部分模型（如 glm 系）调用成功后会把片段忘在工具结果里，
  只在正文写「图在上面」导致用户看不到任何图片。现三重防线：
  - 工具 description 开头显式声明「本工具本身不会把图片显示给用户，不粘贴片段 = 用户什么都看不到」，
    并禁止只说「见上图」而不粘贴；
  - 执行结果的 `note` 升级为醒目警告（⚠️【必做】），并把 markdown 片段直接内嵌进 note 正文，
    模型即使漏看 `markdown` 数组也能从 note 里原样取到片段；
  - 自检 27 项全部通过，行为不变，仅提示文本增强。
